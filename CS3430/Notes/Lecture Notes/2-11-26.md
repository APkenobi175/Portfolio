...
- Block sequence tests also includes observations.
  - Observations are the number of blocks we see in the sequence, e.g. 1, 2, 3.

- $O_i$ - counts number of blocks of length i, e.g. $O_1, O_2, O_3$

##### Formula

- $x^2 = 4M[\sum (S_i - \frac{M}{2})^2]$
- $S_i$ - number of 1s in block i

- $x^2 = \sum_{k=1}^{6} \frac{(O_k - E_k)^2}{E_k}$
- $E_k$ - expected number of blocks of length k, which is given by the distribution of the number of blocks of length k, which is $E_k = \frac{n}{6}$
...
- this is the chi-squared distribution with 5 degrees of freedom, which is what we get from the block sequence test. We can use this to compute the p-value from the chi-squared statistic.
